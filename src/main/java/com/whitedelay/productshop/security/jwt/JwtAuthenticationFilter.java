...
import com.whitedelay.productshop.security.dto.LoginRequestDto;
...
public class JwtAuthenticationFilter extends UsernamePasswordAuthenticationFilter { // UsernamePasswordAuthenticationFilter을 상속받아옴, 세션 방식 말고 jwt쓸거라서 custom하는거임
    ...
    protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain, Authentication authResult) throws IOException, ServletException {
        ...
        String token = jwtUtil.createToken(memberid, role); // 토큰 생성 시 2번째 파라미터로 role을 넣어주기 위함
        jwtUtil.addJwtToCookie(token, response); // 쿠키생성하고 token을 넣어주는 메소드
